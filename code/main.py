...
I need 4 gift ideas with a creativity level of {creativity}/10 for my {gift_profile_path.stem}:
...
{gift_profile_data}"""
    ...
    try:
        ...
        # Automatically run aiBakend.py
        print("\nAutomatically running aiBakend.py to generate gift suggestions...")
        import subprocess
        import os
        
        # Get the absolute path to aiBakend.py in the parent directory
        aibakend_path = Path(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) / "aiBakend.py"
        
        # Run aiBakend.py
        try:
            subprocess.run(["python", str(aibakend_path)], check=True)
        except subprocess.CalledProcessError as e:
            print(f"Error running aiBakend.py: {e}")
        except Exception as e:
            print(f"Unexpected error running aiBakend.py: {e}")
    except Exception as e:
